...
void Handle::init(int timeslot)
{
    m_TIMESLOT = timeslot;
}

//对文件描述符设置非阻塞
int Handle::setnonblocking(int fd)
{
    //fcntl可以获取/设置文件描述符性质
...
//设置信号函数
void Handle::addsig(int sig, void(Handler)(int), bool restart)
{
    struct sigaction sa;
...
int *Handle::u_pipefd = 0;
int Handle::u_epollfd = 0;
...
void cb_func(client_data *user_data)
{
    ...
    epoll_ctl(Handle::u_epollfd, EPOLL_CTL_DEL, user_data->sockfd, 0);
    assert(user_data);
    //删除非活动连接在socket上的注册事件
